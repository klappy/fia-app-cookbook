...
## September 15 pending alignment release

Candidate `c93e29d` (0.1.10) adds source-bound English word and verse timing for the three existing recordings. Three initial alignment requests completed, and native boundary/owner/pause/fallback checks plus an offline cold-page check pass. Its build contains 421 distribution files and 203 required English offline files. Public deployment and exact HTTP readback remain pending; the verified public baseline is the September 14 release above. Audio bytes and the 204 Spanish recordings are preserved.
