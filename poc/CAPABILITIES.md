...
Actual clip identities drive released highlighting. Spanish verse clips support verse-level highlighting; the September 15 release adds word and verse timing to the unchanged full-passage English recordings, with passage-level fallback when timing is unavailable. Automatic synthetic introductions are disabled by operator policy, while unseen source-specific notices remain distinct. Explicit Info replay preserves paused content progress.

## September 15 alignment release

Released main `0c21899` (version 0.1.10) adds source-bound English word and verse timing for BSB, ULT and UST in the shared Scripture view. Three initial forced-alignment requests completed after upload permission was enabled; no retries or replacement recordings were needed. Exact build verification passes 421 distribution files and 203 required English offline files. Release gates passed. Automatic deployment succeeded at 14:32:02.564 UTC; independent HTTP readback at 14:33:21.880409 UTC verified all 421 files and 216,741,866 bytes with zero failures.
