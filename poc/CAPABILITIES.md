...
Release-state snapshot, 2026-09-12 UTC: shared inset controls, resource reading-order corrections and install support are released at app main `97cd571`. A current HTTP check verified five critical release assets; that narrower check is not a new whole-distribution audit. Spanish source projection is accepted; its working language-selection candidate remains in PR12 pending final review and gates, so public Spanish availability is not yet claimed.
...
| Second language | Reviewed Spanish source pack and labeled supplements; working full language-picker candidate in PR12 | Not yet a public Spanish release; narration remains unavailable, and complete audio-first acceptance remains pending |
